...
export async function productHtmlDescriptionFormatter(session) {
  ...
  if(products[1].length > 0) {
    for (const product of products[1]) {
...
export async function getAllProducts(session) {
  ...
  try {
    ...
    const productsWithoutToc = products.filter(product => {
      return !product.node.metafields.edges.some(metafield => metafield.node.value === 'toc is generated');
    });

    return [products , productsWithoutToc]
